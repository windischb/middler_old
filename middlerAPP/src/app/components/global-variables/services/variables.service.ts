...
import { FolderTreeNode, Variable } from '../models';
...
export class VariablesService {
    ...
    private SelectedFolderItemsSubject$: BehaviorSubject<Array<Variable>> = new BehaviorSubject<Array<Variable>>(null);
    ...
    public GetVariablesInSelectedFolder() {
        return this.messages.Invoke<Array<Variable>>("Variables.GetVariablesInParent", this.SelectedFolderSubject$.getValue().Path)
            .pipe(
                take(1),
    ...
    public UpdateVariableContent(parent: string, name: string, content: string) {
        console.log("Variables.UpdateVariableContent", parent, name, content)
        return this.messages.Invoke<Variable>("Variables.UpdateVariableContent", parent, name, content)
            .pipe(
                take(1),
