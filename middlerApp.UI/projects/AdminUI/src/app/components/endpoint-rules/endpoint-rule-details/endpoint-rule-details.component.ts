...
export class EndpointRuleDetailsComponent {
    ...
    public rule$ = combineLatest([this.route.paramMap, this.route.queryParamMap, this.route.fragment]).pipe(
        ...
        mergeAll(),
        map(rule => {
            rule.Actions = rule.Actions.sort((a,b) => a.Order - b.Order)
            return rule;
        }),
        tap(rule => {
            if (!this.BaseRule) {
    ...
    save() {
        ...
        var rule = <EndpointRule>this.form.value;
        rule.Actions?.forEach(act => {
            var nId = filterInt(act.Id);
            if(!Number.isNaN(nId)) {
                act.Id = null;
            }
        })
        ...
        this.rulesService.Update(this.Id, rule).pipe(
            tap(() => {
